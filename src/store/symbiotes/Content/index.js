...
const symbiotes = {
  ...
  decreaseFontSize: (state) => ({
    ...state,
    preferredFontSize: state.preferredFontSize - 1,
  }),
  ...
  setArticles: (state, payload) => {
    ...
    if (state.dailyList[day] !== undefined) {
      ...
      dailyList = Object.values(articles)
        .filter((article) => {
          ...
          if (
            hasTags === true &&
            state.pickedList.includes(article.id) === false &&
            state.bookmarkedList.includes(article.id) === false &&
            Object.keys(state.currentlyReadingList).includes(article.id) === false &&
            Object.keys(state.doneReadingList).includes(article.id) === false
          ) {
    ...
    return {
      ...
      pickedList: [...state.pickedList, ...newPickList],
      exploreSearchIndex: {
        ...state.exploreSearchIndex,
        ...searchIndex,
      },
      renderQueryExplore: [...Object.keys(articles)],
    }
  },
  ...
  addToBookmarked: (state, payload) => ({
    ...state,
    bookmarkedList: [...state.bookmarkedList, payload],
    bookmarkedSearchIndex: {
  ...
  completeArticle: (state, payload) => {
    ...
    return {
      ...
      doneReadingList: [...state.doneReadingList, payload],
    }
  },
