    ...
    return <Carousel
      ...
      enableMomentum={false}
       ...
       renderItem={({ item, index }) => {
         return <CardCurrentlyReading
           key={index}
           theme={theme}
           translation={translation}
           category={t(`topics.${articles[item].category}`, translation)}
           host={articles[`${item}`].host}
           publication={articles[`${item}`].publicationDate}
           title={articles[`${item}`].title}
           image={articles[item].cover['2x'].url}
           percentLeft={list[item] === undefined || list[item].progression === 0 ? 100 : Math.ceil(100 - list[item].progression)}
           timeLeft={list[item] === undefined || list[item].progression === 0 ? articles[`${item}`].durationInMinutes : Math.ceil(list[item].progression / 100 * articles[`${item}`].durationInMinutes)}
